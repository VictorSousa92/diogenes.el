               ...
               (or said
                   (let* ((labels
                           (let ((n 0))
                             (mapcar (lambda (book)
                                       (setq n (1+ n))
                                       (cons (diogenes-books--label book n)
                                             book))
                                     books))))
                     ;; SHOWN AND NOT COUNTED.  Every book with its letter,
                     ;; its number and the page it starts at, so a reader
                     ;; chooses by looking -- which is the only way to be
                     ;; right about a work whose numbering and whose letters
                     ;; disagree.
                     (car (split-string
                           (completing-read
                            "Book: "
                            ;; IN THE WORK'S ORDER, not the alphabet's.
                            ;; Completion sorts what it is given, and sorted
                            ;; alphabetically the list read Α ΕΛΑΤΤΟΝ, Β, Ε,
                            ;; Η, Γ -- which is no order at all to choose a
                            ;; book from.  `identity' as the sort leaves them
                            ;; as they were built, which is the order the work
                            ;; prints them in.
                            ...
                            nil t)
                           "  " t)))))
              (book (diogenes-books--choose books choice)))
         (unless book
