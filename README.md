...
- [Additional features (Victor)](#additional-features-victor)
  ...
  - [The dictionaries themselves](#the-dictionaries-themselves)
    ...
    - [TGL Volume V (special menu)](#tgl-volume-v-special-menu)
  ...
  - [Notes joined to passages (org)](#notes-joined-to-passages-org)
    - [The form of a reference](#the-form-of-a-reference)
    - [Storing a link](#storing-a-link)
    - [Following one back](#following-one-back)
    - [The notes on what you are reading](#the-notes-on-what-you-are-reading)
    - [Making one](#making-one)
    - [Back from a note](#back-from-a-note)
    - [The keys](#the-keys)
    - [What the rest of the branch carries](#what-the-rest-of-the-branch-carries)
...
- Notes in org-roam anchored to citations rather than to page numbers, with links both ways.
...
## Notes joined to passages (org)

*On the `org-integration` branch only.*

A note belongs to a passage, and a passage does not move. That is the whole
argument for what follows, and the reason none of it uses `org-noter`.

`org-noter` anchors a note to a place in a document — `:NOTER_PAGE: 114` —
which means nothing without the PDF it was taken against, and nothing at all
when you read Slings instead of Burnet. `327a5` means the same thing in every
edition there has ever been, and will mean it when the scan is lost. So the
citation is the anchor.

What this half adds:

- A `diogenes:` link type, stored from the browser and followed back to it.
- Notes found by citation rather than by text, so a note made on a stretch is found from anywhere inside it.
- Commands both ways: the notes on what you are reading, and the passage a note is about.

Nothing here is needed to read the notes afterwards. The reference is a
string you can `grep`, and a link falls back to its description, so
`[[diogenes:...][Arist. Metaph. 1048a27]]` reads as *Arist. Metaph. 1048a27*
in any Emacs and as plain text in none.

### The form of a reference

```
diogenes:passage:tlg:0086:025:1048a.27-1048b.16
         kind    corpus author work from    -to
```

- The **kind** is `passage`, `entry` (a dictionary headword) or `page` (a leaf of a printed edition), so a link to a note on a word is not mistaken for one to a text.
- The **corpus and the author** are there because a bare `327a` is ambiguous: Plato's *Republic* and half a dozen other works have one. Without them a note on the *Republic* would be offered for anything numbered alike.
- The **citation** is the levels with stops between them, as the corpus takes them, and a range where a range was marked.

### Storing a link

Mark the passage in the browser and store a link the ordinary org way — `C-c
l`, or whatever `org-store-link` is bound to in your configuration. `C-c C-l`
inserts it in the note.

- With a region marked the link keeps the range; with none it names the line at point.
- A region that begins and ends on the same line gives a single citation and not a range of one place to itself.
- The description is the passage as a scholar writes it, from the abbreviations: *Arist. Metaph. 1048a27*.

### Following one back

`C-c C-o` on the link opens the passage — and if a browser is already reading
that work, that browser is used rather than another being started.

- The lines the note was made on are **marked** on arrival. The Perl is asked and answers in its own time, so the marking waits for the text and then retries a few times before giving up.
- `diogenes-org-reuse-browser` nil opens a fresh browser every time instead. Reuse cannot page a browser to a distant passage — paging is the Perl's business, not the buffer's — so where the passage is not in the stretch already shown, it says so rather than pretending.

### The notes on what you are reading

`C-c C-y` in a browser looks for notes on the whole of what is displayed, not
on the line at point: a note two lines above the cursor bears on the page as
much as one on it.

```
2 notes:  1048a.27              Arist. Metaph. 1048a27
          1048a.30-1048a.32     Arist. Metaph. 1048a30-1048a32
```

- **The citation comes first**, because that is what you choose by — not the note's title but the lines it was made on.
- Offered even where there is only one, so you see which lines before opening it.
- A note **meets** the page rather than having to sit inside it: one made on a paragraph that begins before this page and ends on it is offered too.
- Sorted by passage, so the list runs down the page as the text does.
- `C-u C-c C-y` widens to every note on the work, which is what you want on arriving at a dialogue rather than at a line.

Notes are found by comparing citations as numbers, not by matching strings, so
`1048a.27` is found under a note made on `1048a.27-1048b.16` without either
having to be spelled the other's way. `diogenes-org-near` allows a line or two
of slack, for a reader who takes notes on a stretch and then wants them from a
little before it.

### Making one

`C-c C-w` in a browser captures an org-roam note on the passage in hand, or on
the stretch marked. The citation goes into `ROAM_REFS` — which is how the note
is found again — and the title is the passage as a scholar writes it.

- `diogenes-org-title-by-name` non-nil titles it with the work's own name, as the corpus gives it: *Metaphysica 1048a27*. Nil uses the dictionaries' abbreviations instead, *Arist. Metaph. 1048a27*, which is what one writes in a footnote.
- `diogenes-org-capture-template` replaces the built-in template, for a reader who files notes by author and work rather than by date.

### Back from a note

`C-c C-x C-d` in an org buffer opens the passage the note is about, read from
its own `ROAM_REFS` — so it works anywhere in the note, and in notes written
before there were links to click.

The reference is looked for outwards from point: the entry's own, then each
heading above it, then the file's, then a link in the body. The nearest one
that names a passage wins, an inner note being about a narrower thing than its
parent.
...
| Key | Where | What |
| --- | --- | --- |
| `C-c C-y` | browser | the notes on what is displayed |
| `C-c C-w` | browser | make a note on it |
| `C-c C-x C-d` | org | the passage this note is about |

Diogenes has taken much of `C-c` and a letter already — `C-c C-a` to the
analysis, `C-c C-b` to the browser, `C-c C-c` for a lookup, `C-c C-o` for the
dictionary, `C-c C-n` and `C-c C-p` to page, `C-c C-q` to quit, `C-c C-t` for
the citations — hence the corners these sit in.
`diogenes-org-notes-key`, `diogenes-org-note-key` and
`diogenes-org-goto-key` change them, and a key already bound to something else
is left alone and said so rather than taken silently.

Under Doom the localleader is roomier:

```elisp
(setq diogenes-org-notes-key nil
      diogenes-org-note-key  nil
      diogenes-org-goto-key  nil)
(map! :map diogenes-browser-mode-map :localleader
      "n" #'diogenes-org-notes
      "w" #'diogenes-org-note)
(map! :map org-mode-map :localleader
      "d" #'diogenes-org-goto-passage)
```

### What the rest of the branch carries

The links rest on a citation layer that is not only for org, and which the
critical-editions package uses as well:

- `diogenes-browser-reference` — the passage in hand, as a plist: its levels, its labels, the work and author, and the citation as a string.
- `diogenes-browser-citation-at` and `-citation-interval` — the citation of a line, and of a marked region.
- `diogenes-citation-to-key`, `-from-key`, `-to-string` — a citation as a number for comparing, and as prose for reading.
- `diogenes-open-passage` and `diogenes-open-reference` — a work opened at a place.
- `diogenes-abbreviations.el` — the LSJ and Lewis & Short abbreviations, so a passage can be named *Arist. Metaph.* rather than *tlg 0086/025*. Optional: without it the numbers are used and nothing breaks. `diogenes-abbreviation-overrides` corrects or adds an entry.
