...
It can then be edited, saved to file and called directly for better testing."
  ;; THROUGH THE LAYER, like everything else that displays a buffer.  It is a
  ;; scratchpad and it wants to be in front of the reader, hence
  ;; `same-window'; but a reader who would rather debug in another frame
  ;; should be able to say so in one place, and one function placing its own
  ;; buffer is how that stops being true.
  (classicist-display-buffer (generate-new-buffer "*diogenes-perldebug*")
                             :kind 'debug :same-window t)
  (insert script)
  (cperl-mode))
