...
;; HERE, AND NOT IN `diogenes.el'.  Both were defined in the entry point and
;; read from this file, which the entry point requires -- so this file called
;; UP into the file that loads it, and got away with it only because the calls
;; happen at run time, by which point everything is loaded.  Compiled on its
;; own, it reported fourteen references to variables it had never heard of.
;;
...
;; THE `-I' FLAGS, and they belong beside the invocation that uses them: five
;; calls here and two in `diogenes-archive.el', nowhere else.  They were in
;; `diogenes.el', which requires this file, so this file called up into the
;; one that loads it -- the same shape as the two variables above, and fixed
;; the same way.
;;
;; `diogenes--path' STAYS THERE, and is declared instead.  It reads
;; `diogenes-path', the one thing a reader has to set, and where the
...
(defun diogenes--browse-interactively-script (option-plist passage)
  ...
  (diogenes--perl-script
   ...
   ;; screen, and moving `$beg' to the start of the block just added makes `p'
   ;; fetch that block again.  Turning, `$beg' must follow: the first line is
   ;; gone from the buffer, and leaving `$beg' behind makes Perl go on
   ;; believing the reader is at the beginning -- so `p' after turning forward
   ;; from line 1 answers that there is nothing before.
   "  elsif (/^F$/) {"
